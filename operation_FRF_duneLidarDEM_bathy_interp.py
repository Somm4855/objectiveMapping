...
import objMapPlots
import sys
sys.path.append(r"C:/Users/DoD_Admin/Documents/ERDC_FRF/pyCMTB")
...
from objectiveMapping.objMapPrep import coarseBackground, binMorph
from getdatatestbed import getDataFRF
from testbedutils import sblib as sb
import objectiveMapping.objMapPlots
...
###########
# set start, end
start, end = DT.datetime(2016, 4, 28), DT.datetime(2016, 5, 13)
...
for tt, date in enumerate(sb.createDateList(start, end, dt)):   # make one merged product (hourly)
    ...
    if bathyAll is not None:
        if (type(bathyAll['time']) == np.ma.core.MaskedArray) and (bathyAll['time'].mask == False):
            bathyAll['time'] = np.array(bathyAll['time'])
